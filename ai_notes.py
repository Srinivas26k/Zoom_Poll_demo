...
class AINotesGenerator:
    ...
    def generate_note(self, transcript_segment: str) -> MeetingNote:
        ...
        try:
            if self.client:
                ...
                try:
                    if self.use_llama:
                        ...
                        content = response.choices[0].message.content
                except APIError as api_err: # Specific catch for openai errors (now just APIError)
                    logger.error(f"OpenAI API error while generating note: {api_err}", exc_info=True)
                    content = None # Ensure content is None so fallback is triggered
                
                # Parse JSON response if content was fetched
                if content:
                    try:
                    ...
                    except Exception as e_parse: # Renamed e to e_parse to avoid conflict
                        logger.error(f"Error parsing AI response for note: {e_parse}", exc_info=True)
                        content = None # Set content to None to trigger fallback if parsing fails
            
            # Fallback to simple note generation if client is None, content is None (API error), or JSON parsing failed
            if not self.client or not content: # Check if we need to fallback
            ...
            logger.error(f"Error generating note: {str(e)}", exc_info=True) # Added exc_info=True
            return MeetingNote(
                timestamp=datetime.now().strftime("%H:%M:%S"),
                content=f"Error generating note: {str(e)}", # Kept original error content for MeetingNote
                tags=["error"]
            )
    ...
    def extract_action_items(self, transcript: str) -> List[ActionItem]:
        ...
        try:
            if self.client:
                ...
                try:
                    if self.use_llama:
                        ...
                        content = response.choices[0].message.content
                except APIError as api_err: # Changed to APIError
                    logger.error(f"OpenAI API error while extracting action items: {api_err}", exc_info=True)
                    content = None # Ensure content is None so fallback is triggered
                ...
                if content:
                    try:
                        ...
                        if json_match:
                            ...
                            action_items_result = [] # Renamed to avoid conflict
                            for item in data.get("action_items", []):
                                action_items_result.append(ActionItem(
                        ...
                        logger.error(f"Error parsing action items JSON: {e_parse}", exc_info=True)
                        content = None # Set content to None to trigger fallback if parsing fails
            
            # Simple fallback extraction if client is None, content is None, or JSON parsing failed
            ...
            logger.error(f"Error extracting action items: {str(e)}", exc_info=True) # Added exc_info=True
            return []
        ...
        try:
            ...
            if self.client:
                ...
                try:
                    ...
                    logger.error(f"OpenAI API error while generating meeting summary: {api_err}", exc_info=True)
                    content = None # Ensure content is None so fallback is triggered
                ...
                if content:
                    try:
                        ...
                        logger.error(f"Error parsing meeting summary JSON: {e_parse}", exc_info=True)
                        content = None # Set content to None to trigger fallback if parsing fails
            
            # Simple fallback summary if client is None, content is None, or JSON parsing failed
            if not self.client or not content:
            ...
            logger.error(f"Error generating meeting summary: {str(e)}", exc_info=True) # Added exc_info=True
            return MeetingSummary(
                meeting_id=meeting_id,
                date=datetime.now().strftime("%Y-%m-%d"),
                title=f"Meeting {meeting_id}", # Kept original title format for error
                summary=f"Error generating summary: {str(e)}", # Kept original summary format for error
                action_items=[],
                notes=[]
            )
